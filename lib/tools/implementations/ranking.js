...
function formatPlayerCompact(p) {
    ...
    return `[#${p.rank}] | ${p.name} | ${p.score.toLocaleString()} | +${speed} | +${speed_3h}`;
}
...
[榜線列表 (格式: [排名] | 玩家名稱 | 分數 | 時速 | 3h均速)]:
${rankingsStr}

[⚠️ 資料解讀規則 (Data Context)]:
1. 此為篩選後的數據，僅包含第 ${finalStart} 名到第 ${finalEnd} 名。
2. **時速資訊**：此資料包含玩家名稱、即時時速 (+xxx) 和 3h均速 (+xxx)。
3. **玩家名稱**：玩家名稱必須**完全**保留所有字元，不可任意修改或翻譯。
        `.trim();
    } catch (error) {
